  ...
  for(case in 1:3){
    ...
    plot <- ggplot(data, aes(x = gene, y = t, fill = comparison, color = comparison)) + 
      ...
      labs(title = paste("Case", case), 
           x = "", y = "Change in expression", fill = "Comparison") + 
...
Not surprisingly, highly-expressed and unique genes in condition [**A**]{style="color: #EE7AE9"} have significantly higher expression when compared to the other two conditions, and except for a few false positives, the rest of genes don't present significant expression changes.

Now, as you may be suspecting, the decrease in counts for genes in condition [**A**]{style="color: #EE7AE9"} after sampling results in false discoveries of negative differential expression.
...
We cannot estimate $S_k$ and $S_r$ but we can take advantage of the inverse relationship between $\frac{S_k}{S_r}$ and the fold changes of non-DEGs, to approximate it by computing a [**global mean expression fold change across all (assumed) non-DEGs**]{style="background-color: #FFFFE0"} for sample $k$ vs $r$.
...
What Mark D. Robinson and Alicia Oshlack proposed with their [**Trimmed Mean of M-values**]{style="background-color: #FFFF00"} method is an empirical approach to approximate $\frac{S_r}{S_k}$, which can be summarized as follows:

1.  Compute gene-wise log-normalized expression ratios between samples $k$ and $r$ (logFCs):
    ...
    Below we compute the $M$ and $A$ values for all genes in each case, comparing sample 1 of condition [**A**]{style="color: #EE7AE9"} vs sample 5 of condition [**B**]{style="color: #7CCD7C"}. We then plot $M$ vs $A$ values to spot up and down differentially expressed genes.
...
3.  Estimate the average of the $M$ values for not trimmed genes ($G^*$), each weighted by gene inverse variances ($w_{gk}^r$) to account for smaller variances in logFCs of genes with larger read counts.
    ...
    In the next toggle code we manually find this factor and compare it with the one returned by `edgeR::calcNormFactors()` and with the true known value of $\frac{S_r}{S_k}$.
    ...
    Note that in case 2 and 3 the factors we compute manually don't match with the ones returned by `calcNormFactors()` because we transformed 0 counts into 1s, whereas this *edgeR* [@edgeR-2] function removes genes with zero counts.
...
| Robinson, M.D. & Oshlack, A. (2010). A scaling normalization method for differential expression analysis of RNA-seq data. *Genome Biol* 11, R25. [https://doi.org/10.1186/gb-2010-11-3-r25](#0)
:::
